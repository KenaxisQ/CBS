...
class CbsAppSupabaseUser extends BaseAuthUser {
  CbsAppSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> cbsAppSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = CbsAppSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
